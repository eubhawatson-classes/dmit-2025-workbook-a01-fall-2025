...
function validate_date($date) {
    ...
    // Even if the user didn't enter their date in this format, we'll see if have the correct information and if we can arrange it in this format. 
    $parsed_date = date_parse_from_format($date_format, $date);

    // Finally, let's check to see if there were any errors and if this date is a date that exists in the Gregorian calendar.
    return $parsed_date['error_count'] === 0 && checkdate($parsed_date['month'], $parsed_date['day'], $parsed_date['year']);
}

// This function calculates the days difference (in days) between two dates (today and the user's provided date).
function calculate_days_difference($date) {
    // Let's start by asking the server what the current date is.
    $current_date = date('Y-m-d');

    // Dates are technically strings! However, unix time is an integer. So, we need to convert our string to unix time in order to do any calculations. 
    $difference = strtotime($current_date) - strtotime($date);

    // Alright, now we have the difference between our two dates — but it's in seconds (unix time)! So, we need to now convert this to days.
            ...
            <div class="col-md-8 col-md-6">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if (validate_date($date)) {
                        ...
                        // If the date is valid, we can go ahead and compare today's date to the user's provided date.
