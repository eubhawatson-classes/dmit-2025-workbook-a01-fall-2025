...
// Retrieve the country name and ranking from the query string, then sanitise it!
...
// If we're missing a value in the query string, we'll display an error message right away.
if ($rank == "" || $country == "") {
    ...
    /*
        ...
        A prepared statement is a template for the kind of query we're going to be using on this page. It uses a ? (a placeholder) value instead of the raw value (ex. $rank). This gives us an additional layer of security (via level of abstraction) and makes us substantially less vulnerable to attacks like SQL injections.
    ...
    */
    ...
    // Because we are no longer doing trivial queries (i.e. where the query is identical every single time) and we're using the user's choices in our query (stored in our super vulnerable $_GET array), we have to use a prepared statement. 
    $query = "SELECT * FROM happiness_index WHERE `rank` = ?;";

    // First, we're check to see if the connection is good. We are 'preparing' our statement.
    if ($statement = $connection->prepare($query)) { // This returns as T or F.
        ...
        if ($row = $result->fetch_assoc()) {
            echo "<h2 class=\"display-5 mb-3\">" . $row['country'] . " Statistics</h2>";
    ...
    } else { // what if preparing our statement fails?
