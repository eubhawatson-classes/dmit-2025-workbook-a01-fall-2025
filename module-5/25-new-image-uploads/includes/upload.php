...
if (isset($_POST['img-title'], $_POST['img-description'], $_FILES['img-file'])) {

    $file_name = $_FILES['img-file']['name'];
    $file_temp_name = $_FILES['img-file']['tmp_name'];
    $file_size = $_FILES['img-file']['size'];

    // Let's check to make sure that we have an error code of 0 (UPLOAD_ERR_OK), which means everything was uploaded properly.
    if ($_FILES['img-file']['error'] === 0) {

        // Next, let's grab the uploaded file's extension.
        ...
        if (in_array($file_extension, $allowed)) {
            ...
            if ($file_size < 2000000) {
                ...
                // This function will generate a unique filename for us, based upon microseconds. This helps us avoid data collisions on larger platforms or applications. 
                $file_name_new = uniqid('', TRUE) . ".$file_extension";
                $file_destination = "images/full/$file_name_new";

                // Let's make sure that the necessary directories (i.e. where we are going to store our final images) exists and that PHP has read/write permissions.
                ...
                // Move the uploaded file out of the temporary files and into the directory that we now know exists. 
        ...
        // If the error status != 0 ... 
        $message .= "There was an error with your file. Please make sure it isn't corrupted and try uploading again later.";
    }

} elseif (isset($_POST['submit'])) {
    // If the form is submitted with something missing ...
    $message .= "Please provide a title, a description, and an image file.";
}
